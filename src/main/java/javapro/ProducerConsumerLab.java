...
public class ProducerConsumerLab {
    ...
    static class Producer implements Runnable {
        private SharedBuffer buffer; // Adds private SharedBuffer field

        // Constructor
        public Producer(SharedBuffer buffer) {
            this.buffer = buffer; // Initializes the buffer field
        }

        @Override
        public void run() {
            try {
                for (int i = 0; i < 10; i++) // Loops 10 times, from 0 to 9
                {
                    buffer.produce(i);
                }
                System.out.println("[Producer] finished producing 10 items"); // Prints once code finishes running
            } 
    ...
    static class Consumer implements Runnable {
        ...
        public void run() {
            try {
                ...
                System.out.println("[Consumer] finished consuming 10 items"); // Prints once code finishes running
            } 
            catch (InterruptedException e) {
